...
// TODO by CuongNT: Can sua ham nay thanh don gian hon. Vi ban chat ham nay k can phuc tap the nay.
const isPayloadWithStateKey = (payload) => {
    let flag = false;
    const rootKeys = Object.keys(payload);
    rootKeys.every((rootKey) => {
        if (!payload[rootKey]) return true; // Key không có data bên trong
        ...
        if (isImmutable(payload[rootKey])) {
            ...
            branchKeys.every((branchKey) => {
                ...
                if (branch) {
                    ...
                    flag = leafKeys.includes('data') || leafKeys.includes('itemIds') || leafKeys.includes('items');
        ...
        const branchKeys = Object.keys(typeof(payload[rootKey]) === 'object' ? payload[rootKey] : {});
        branchKeys.length > 0 && branchKeys.every((branchKey) => {
